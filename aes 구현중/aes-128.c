//AES C coded
#include <stdio.h>
#include "aes.h"

#define KEY_SIZE_128 16
#define KEY_SIZE_192 24
#define KEY_SIZE_256 32

#define Nr_128 10
#define Nr_192 12
#define Nr_256 14

static unsigned int sbox[256] = {

};

static unsigned int inv_sbox[256] = {

};

static unsigned int Rcon[100] = {
    0x01, 0x00, 0x00, 0x00,
    0x02, 0x00, 0x00, 0x00,
    0x04, 0x00, 0x00, 0x00,
    0x08, 0x00, 0x00, 0x00,
    0x10, 0x00, 0x00, 0x00,
    0x20, 0x00, 0x00, 0x00,
    0x40, 0x00, 0x00, 0x00,
    0x80, 0x00, 0x00, 0x00,
    0x1b, 0x00, 0x00, 0x00,
    0x36, 0x00, 0x00, 0x00
}; // key schedule 연산에 사용되는 상수

unsigned int x_time(unsigned int x) {

    x = x * 0x02;

    if (x > 0xff)
        return ((x << 24) >> 24) ^ 0x1b;
    else
        return x;
}

void RotWord(unsigned int* temp, unsigned int* word[]) {

    temp[0] = word[KEY_SIZE_128-3];
    temp[1] = word[KEY_SIZE_128-2];
    temp[2] = word[KEY_SIZE_128-1];
    temp[3] = word[KEY_SIZE_128-4];

}

void SubWord(unsigned int* temp, unsigned int* sbox) {
    for (int j = 0; j < 4; j++) // Subword
        temp[j] = sbox[temp[j]];
}

void AddRoundKey(unsigned int text[], unsigned int roundkey[], unsigned int round) {
    for (int j = 0; j < KEY_SIZE_128; j++)
        text[j] = text[j] ^ roundkey[KEY_SIZE_128 * round + j];
}

void SubBytes(unsigned int plaintext[], unsigned int s_box[]) {
    for (int j = 0; j < KEY_SIZE_128; j++) // 평문 바이트 s-box 에 전치시키기
        plaintext[j] = s_box[plaintext[j]];
}

void ShiftRows(unsigned int* plaintext) {
    unsigned int t[KEY_SIZE_128] = { 0, };

    for (int j = 0; j < KEY_SIZE_128; j++)
        t[j] = plaintext[j];

    for (int j = 0; j < KEY_SIZE_128; j++)
        plaintext[j] = t[(5 * j) % KEY_SIZE_128];
}

void MixColumns(unsigned int* plaintext) {

    unsigned int t[KEY_SIZE_128] = { 0, };

    for (int j = 0; j < 4; j++) {
        t[4 * j + 0] = x_time(plaintext[4 * j + 0]) ^ (x_time(plaintext[4 * j + 1]) ^ plaintext[4 * j + 1]) ^ (plaintext[4 * j + 2]) ^ (plaintext[4 * j + 3]);
        t[4 * j + 1] = (plaintext[4 * j + 0]) ^ x_time(plaintext[4 * j + 1]) ^ (x_time(plaintext[4 * j + 2]) ^ plaintext[4 * j + 2]) ^ (plaintext[4 * j + 3]);
        t[4 * j + 2] = (plaintext[4 * j + 0]) ^ (plaintext[4 * j + 1]) ^ x_time(plaintext[4 * j + 2]) ^ (x_time(plaintext[4 * j + 3]) ^ plaintext[4 * j + 3]);
        t[4 * j + 3] = (x_time(plaintext[4 * j + 0]) ^ plaintext[4 * j + 0]) ^ (plaintext[4 * j + 1]) ^ (plaintext[4 * j + 2]) ^ x_time(plaintext[4 * j + 3]);
    }

    for (int j = 0; j < KEY_SIZE_128; j++)
        plaintext[j] = t[j];
}


void InvShiftRows(unsigned int* ciphertext) {
    unsigned int t[KEY_SIZE_128] = { 0, };

    for (int j = 0; j < KEY_SIZE_128; j++)
        t[j] = ciphertext[j];

    for (int j = 0; j < KEY_SIZE_128; j++)
        ciphertext[j] = t[(13 * j) % KEY_SIZE_128];
}

void InvSubBytes(unsigned int ciphertext[], unsigned int inv_s_box[]) {
    for (int j = 0; j < KEY_SIZE_128; j++) // 평문 바이트 s-box 에 전치시키기
        ciphertext[j] = inv_s_box[ciphertext[j]];
}

void InvMixColumns(unsigned int* plaintext) {

    unsigned int t[KEY_SIZE_128] = { 0, };

    for (int j = 0; j < 4; j++) {
        t[4 * j + 0] = (x_time((x_time(x_time(plaintext[4 * j + 0]) ^ plaintext[4 * j + 0])) ^ plaintext[4 * j + 0])) ^ (x_time(x_time(x_time(plaintext[4 * j + 1])) ^ plaintext[4 * j + 1]) ^ plaintext[4 * j + 1]) ^ (x_time(x_time(x_time(plaintext[4 * j + 2]) ^ plaintext[4 * j + 2])) ^ plaintext[4 * j + 2]) ^ (x_time(x_time(x_time(plaintext[4 * j + 3]))) ^ plaintext[4 * j + 3]);
        t[4 * j + 1] = (x_time(x_time(x_time(plaintext[4 * j + 0]))) ^ plaintext[4 * j + 0]) ^ (x_time((x_time(x_time(plaintext[4 * j + 1]) ^ plaintext[4 * j + 1])) ^ plaintext[4 * j + 1])) ^ (x_time(x_time(x_time(plaintext[4 * j + 2])) ^ plaintext[4 * j + 2]) ^ plaintext[4 * j + 2]) ^ (x_time(x_time(x_time(plaintext[4 * j + 3]) ^ plaintext[4 * j + 3])) ^ plaintext[4 * j + 3]);
        t[4 * j + 2] = (x_time(x_time(x_time(plaintext[4 * j + 0]) ^ plaintext[4 * j + 0])) ^ plaintext[4 * j + 0]) ^ (x_time(x_time(x_time(plaintext[4 * j + 1]))) ^ plaintext[4 * j + 1]) ^ (x_time((x_time(x_time(plaintext[4 * j + 2]) ^ plaintext[4 * j + 2])) ^ plaintext[4 * j + 2])) ^ (x_time(x_time(x_time(plaintext[4 * j + 3])) ^ plaintext[4 * j + 3]) ^ plaintext[4 * j + 3]);
        t[4 * j + 3] = (x_time(x_time(x_time(plaintext[4 * j + 0])) ^ plaintext[4 * j + 0]) ^ plaintext[4 * j + 0]) ^ (x_time(x_time(x_time(plaintext[4 * j + 1]) ^ plaintext[4 * j + 1])) ^ plaintext[4 * j + 1]) ^ (x_time(x_time(x_time(plaintext[4 * j + 2]))) ^ plaintext[4 * j + 2]) ^ (x_time((x_time(x_time(plaintext[4 * j + 3]) ^ plaintext[4 * j + 3])) ^ plaintext[4 * j + 3]));

    }
    for (int j = 0; j < KEY_SIZE_128; j++)
        plaintext[j] = t[j];

}

void AES_128_Encryption(unsigned int roundkey, unsigned int* plaintext, unsigned int* ciphertext) {
    //Initail Round 
    AddRoundKey(plaintext, roundkey, 0);

    //1~9 Round 
    for (int i = 1; i < Nr_128; i++) {

        SubBytes(plaintext, sbox);
        ShiftRows(plaintext);
        MixColumns(plaintext);
        AddRoundKey(plaintext, roundkey, i);

    }

    //Final round
    SubBytes(plaintext, sbox);
    ShiftRows(plaintext);
    AddRoundKey(plaintext, roundkey, 10);

    for (int j = 0; j < KEY_SIZE_128; j++) // 평문 암호문 배열에 옮겨주기
        ciphertext[j] = plaintext[j];
}

void AES_128_Decryption(unsigned int roundkey, unsigned int* ciphertext, unsigned int* plaintext) {

    AddRoundKey(ciphertext, roundkey, 10);

    // round 9 ~ 1
    for (int i = Nr_128 - 1; i > 0; i--) {
        InvShiftRows(ciphertext);
        InvSubBytes(ciphertext, inv_sbox);
        AddRoundKey(ciphertext, roundkey, i);
        InvMixColumns(ciphertext);
    }

    //final round
    InvShiftRows(ciphertext);
    InvSubBytes(ciphertext, inv_sbox);
    AddRoundKey(ciphertext, roundkey, 0);

}

void AES_128_KeyExpansion(unsigned* key, unsigned int* roundkey) {

    unsigned int w[KEY_SIZE_128] = { 0, };
    unsigned int temp[4] = { 0, };

    for (int j = 0; j < KEY_SIZE_128; j++)
        roundkey[j] = key[j];


    for (int i = 1; i < Nr_128 + 1; i++) {
        for (int j = 0; j < KEY_SIZE_128; j++)
            w[j] = roundkey[(KEY_SIZE_128 * (i - 1)) + j];

        RotWord(temp, w);

        SubWord(temp, sbox);

        //SubWord XOR Rcon
        for (int j = 0; j < 4; j++)
            temp[j] = temp[j] ^ Rcon[(4 * (i - 1)) + j];

        for (int j = 0; j < KEY_SIZE_128; j++) {
            if (j < 4)
                roundkey[KEY_SIZE_128 * i + j] = roundkey[(KEY_SIZE_128 * (i - 1)) + j] ^ temp[j];
            else
                roundkey[KEY_SIZE_128 * i + j] = roundkey[(KEY_SIZE_128 * (i - 1)) + j] ^ roundkey[(KEY_SIZE_128 * i - 4) + j];
        }
    }
}